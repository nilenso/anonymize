...
/**
 * Tests for the MaskAnonymizer strategy, which replaces PII entities with asterisks or custom mask
 * text.
 */
public class MaskAnonymizerTest {
  ...
  public void testDefaultMasking() {
    ...
    assertFalse(
        result.getAnonymizedText().contains("support@example.com"),
  ...
  public void testMaskingPreservesContext() {
    ...
    assertTrue(
        result.getAnonymizedText().startsWith("Please contact "),
        "Text before the PII should be preserved");
    assertTrue(
        result.getAnonymizedText().endsWith(" for help."),
