...
public class TagAnonymizerTest {
  ...
  public void testSimpleTagReplacement() {
    ...
    if (!result.getDetectedEntities().isEmpty()) {
      ...
      assertEquals(PIIType.EMAIL, entity.getType(), "Entity should be an EMAIL");
      assertEquals(
          "john.doe@example.com", entity.getText(), "Entity should contain the exact email");
    }

    // Verify original email is not present in output
    assertFalse(
        result.getAnonymizedText().contains("john.doe@example.com"),
  ...
  public void testMultipleTagReplacements() {
    ...
    assertTrue(result.getDetectedEntities().size() > 0, "Should detect at least one email");

    // Verify entities are correctly typed
    for (PIIEntity entity : result.getDetectedEntities()) {
      assertEquals(PIIType.EMAIL, entity.getType(), "All entities should be of type EMAIL");
      assertTrue(entity.getText().contains("@"), "All entities should contain @ symbol");
    }

    // Verify no original emails remain in output
    assertFalse(
        result.getAnonymizedText().contains("@example.com"),
  ...
  public void testMixedEntityTagging() {
    ...
    assertFalse(
        result.getAnonymizedText().contains("email1@example.com"),
    ...
    for (PIIEntity entity : result.getDetectedEntities()) {
      if (entity.getType().equals(PIIType.EMAIL)) {
        hasEmailEntity = true;
        assertEquals(
            "email1@example.com", entity.getText(), "Email entity should preserve original text");
      }
    }
