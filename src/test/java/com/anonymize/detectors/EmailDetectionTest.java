...
public class EmailDetectionTest {
  ...
  public void testSimpleEmailDetection() {
    ...
    assertEquals(
        ...
        "Should have correct start position");
    assertEquals(
        text.indexOf("user@example.com") + "user@example.com".length(),
        entity.getEndPosition(),
        "Should have correct end position");
    ...
    assertTrue(
        result.getAnonymizedText().startsWith("Please contact me at "),
  ...
  public void testMultipleEmails() {
    ...
    assertFalse(
        result.getAnonymizedText().contains("@example.com"), "First email should be masked");
    assertFalse(
        result.getAnonymizedText().contains("@example.org"), "Second email should be masked");
    ...
    for (int i = 0; i < result.getDetectedEntities().size() - 1; i++) {
      ...
      assertTrue(
          current.getEndPosition() <= next.getStartPosition(),
  ...
  public void testComplexEmail() {
    ...
    PIIEntity entity = result.getDetectedEntities().get(0);
    assertEquals(
        "john.doe+label@company-name.co.uk", entity.getText(), "Should extract exact email");
    ...
    assertTrue(
        result.getAnonymizedText().startsWith("My work email is "),
        "Text before email should be preserved");
  }
}
