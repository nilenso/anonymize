...
public class NERDetectorTest {
  ...
  public void testPersonNameDetection() {
    ...
    try {
      ...
      for (PIIEntity entity : entities) {
        System.out.println(
            "  - " + entity.getText() + " (confidence: " + entity.getConfidence() + ")");
      }
      ...
      for (PIIEntity entity : entities) {
        assertEquals(PIIType.PERSON_NAME, entity.getType(), "Entity should be PERSON_NAME type");
        assertTrue(entity.getConfidence() > 0, "Confidence should be greater than 0");
      }
    } catch (Exception e) {
  ...
  public void testLocationDetection() {
    ...
    try {
      ...
      // Verify detected entities are of correct type
      for (PIIEntity entity : entities) {
        assertEquals(PIIType.LOCATION, entity.getType(), "Entity should be LOCATION type");
        assertTrue(entity.getConfidence() > 0, "Confidence should be greater than 0");
      }
    } catch (Exception e) {
