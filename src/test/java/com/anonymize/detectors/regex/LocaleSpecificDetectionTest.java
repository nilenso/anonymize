...
public class LocaleSpecificDetectionTest {
  ...
  public void testEmailDetectionAcrossLocales() {
    ...
    for (Locale locale : locales) {
      ...
      // Print results
      System.out.println(
          "Email detection with locale " + locale + ": " + result.size() + " entities");
      ...
      if (!result.isEmpty()) {
        ...
        assertEquals("test@example.com", entity.getText(), "Entity should contain the exact email");
      }
    }
  }
  ...
  private void testTextWithLocale(String text, Locale locale) {
    ...
    if (text.contains("@")) {
      ...
      assertTrue(
          result.getDetectedEntities().size() > 0,
